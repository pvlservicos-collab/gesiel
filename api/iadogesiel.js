// GET: devolve a conversa da IA do Gesiel (usado pela página pública /iadogesiel
...
const { DEFAULT_CONFIG, sanitizeConfig } = require('./_iadogesiel');
...
module.exports = async (req, res) => {
  if (req.method === 'GET') {
    ...
    try {
      await ensureSchema();
      const { rows } = await getPool().query('SELECT data, version FROM iadogesiel_config WHERE id = 1');
  ...
  try {
    ...
    if (req.method === 'PUT') {
      ...
      if (version === 0) {
        ...
        const r = await pool.query(
          `INSERT INTO iadogesiel_config (id, data, version) VALUES (1, $1::jsonb, 1)
      ...
      const r = await pool.query(
        `UPDATE iadogesiel_config SET data = $1::jsonb, version = version + 1, atualizado_em = now()
    ...
    if (req.method === 'DELETE') {
      await pool.query('DELETE FROM iadogesiel_config WHERE id = 1');
